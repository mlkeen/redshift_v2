...
from flask import (
    Blueprint, render_template, abort, session, redirect, url_for,
...
from app.models import (
    Panel, Player, Interactable, SpaceObject, LaserMessage,
    MessageTarget, Control, ControlInvite, GameState,
    ScannedBioSample, PowerSource, PowerConsumer, Poll,
    Condition
)
from app.forms import (
    PlayerForm, PanelForm, InteractableForm, SpaceObjectForm,
    ControlInviteForm, ControlRegistrationForm, PollForm
)
from app.email_utils import send_email
from app.panel_handlers import handle_polling_display, handle_biolab_display
from app.helpers import (
    resolve_player, resolve_panel, resolve_interactable,
    build_menu_items, get_fictional_time, get_delay_to_target
)
...
def prune_inactive(panel):
    """Clear stale player/interactable from panel if no activity in the last minute."""
...
def register_control(token):
    ...
    if request.method == 'GET':
        form.email_address.data = invite.email
...
def start_phase():
    if request.method == 'POST':
        ...
        state = GameState.query.first() or GameState()
...
def fusion_trigger():
    ...
    if request.method == "POST":
        ...
        db.session.commit()
        flash(f"{fail_type.capitalize()} failure triggered in {seconds} seconds.", "success")
        return redirect(url_for("main.fusion_trigger"))
...
def control_add_object(object_type):
    ...
    form = FormClass()
    if form.validate_on_submit():
        new_obj = FormClass.model_class()  # Must be set on form class
...
@main.route('/control/player/<int:player_id>/add_condition', methods=['POST'])
@login_required
def control_add_condition(player_id):
    player = Player.query.get_or_404(player_id)
    new_condition = request.form.get('condition_code')
    if new_condition:
        if not player.conditions:
            player.conditions = []
        if new_condition not in player.conditions:
            player.conditions.append(new_condition)
            db.session.commit()
    return redirect(url_for('main.control_edit_list', object_type='players'))
...
@main.route("/api/set_fusion_output", methods=["POST"])
def set_fusion_output():
    ...
    if not fusion_drive:
        return jsonify({"error": "Fusion drive not found"}), 404

    fusion_drive.current_output = min(output, fusion_drive.max_output)
    db.session.commit()
    return jsonify({"success": True})
...
# === PANEL VIEW (INTERACTIVE TERMINALS) ===
...
def panel_view(panel_code, display, player_code, interactable_code):
    ...
    # Dispatch to handler functions for specific displays
    DISPLAY_HANDLERS = {
        "polling": handle_polling_display,
        "biolab": handle_biolab_display,
        # Add more display handlers as needed
    }
    ...
    # Log biological scans
    if interactable and interactable.is_biological and display.lower() == "biolab":
    ...
    if display.lower() == "power_distribution":
        ...
        total_production = sum(s.current_output for s in sources if s.status == "Online")
        total_draw = sum(c.power_draw for c in active_consumers)
    ...
    # Laser message sending
    if request.method == "POST" and request.form.get("form_type") == "send_laser":
        ...
        db.session.commit()
        flash(f"Message sent to {target.name}. ETA: {round(delay_seconds / 60, 2)} min.", "success")
        return redirect(request.path)
